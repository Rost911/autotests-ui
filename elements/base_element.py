...
class BaseElement:
    ...
    def get_locator(self, nth: int = 0, **kwargs) -> Locator:
        ...
        step = (
            f'Getting locator with "data-testid={locator}" '
            f'at index "{nth}"'
        )
    ...
    def get_raw_locator(self, nth: int = 0, **kwargs) -> str:
        return (
            f"//*[@data-testid="
            f"'{self.locator.format(**kwargs)}'][{nth + 1}]"
        )
    ...
    def check_visible(self, nth: int = 0, **kwargs):
        step = (
            f'Checking that {self.type_of} '
            f'"{self.name}" is visible'
        )
